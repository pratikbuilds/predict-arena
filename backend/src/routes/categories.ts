...
export const categoriesRoutes = new Hono<{ Variables: AgentVariables }>();

categoriesRoutes.get("", requireAgent, async (c) => {
  try {
    const res = await dflowGet("/api/v1/tags_by_categories");
