...
import { seriesListQuerySchema } from "../lib/schemas/query-params.js";
import { parseQuery } from "../lib/validation.js";

export const seriesRoutes = new Hono<{ Variables: AgentVariables }>();
...
seriesRoutes.get("", requireAgent, async (c) => {
  const parsed = parseQuery(c, seriesListQuerySchema);
  if (parsed instanceof Response) return parsed;

  try {
    const res = await dflowGet("/api/v1/series", toDflowQuery(parsed));
